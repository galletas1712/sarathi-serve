...
class SchedulerOutputs:
    ...
    def __init__(
        ...
        self.prompt_chunk_lens = [
            metadata.num_prompt_tokens for metadata in scheduled_seq_id_metadata_list
        ]
        self.num_batched_prompt_tokens = sum(self.prompt_chunk_lens)
        self.num_batched_output_tokens = sum(
            metadata.num_output_tokens for metadata in scheduled_seq_id_metadata_list
        )
        self.num_batched_tokens = sum(
            metadata.num_tokens for metadata in scheduled_seq_id_metadata_list
        )
    ...
    def has_no_output(self) -> bool:
        ...
        return not self.scheduled_seq_id_metadata_list

    @property
    def seq_ids(self) -> List[str]:
        return [metadata.seq_id for metadata in self.scheduled_seq_id_metadata_list]
