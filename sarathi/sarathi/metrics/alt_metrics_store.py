...
def calculate_percentile_values(
    data: List[float], percentiles: List[float] = [50, 90, 95, 99, 99.9, 100]
) -> Dict[float, float]:
    """Calculate multiple percentiles from list of values."""
    if len(data) == 0:
        return {}
    return {
        p: float(np.percentile(data, p)) if p < 100 else max(data) for p in percentiles
    }
...
class SwapInterval:
    ...
    def get_total_duration(self) -> float:
        ...
        assert (
            self.finish_swap_in_timestamp is not None
            and self.start_swap_out_timestamp is not None
        )

        # NOTE: Calculating with offset
        return (
            self.finish_swap_in_timestamp
            - self.start_swap_out_timestamp
    ...
    def get_batch_duration(self) -> int:
        ...
        assert (
            self.finish_swap_in_batch_id is not None
            and self.start_swap_out_batch_id is not None
        )

        return (
            self.finish_swap_in_batch_id
            - self.start_swap_out_batch_id
            - self.batch_offset
        )
...
class SequenceMetrics:
    ...
    def start_swap_out(self, batch_id: int, num_tokens: int, start_timestamp: float):
        ...
        if self._curr_swap_interval is None:
            self._curr_swap_interval = SwapInterval(
                start_swap_out_batch_id=batch_id,
                start_swap_out_timestamp=start_timestamp,
            )
        self._curr_swap_interval.swap_out_lens.append(num_tokens)

    def start_swap_in(self, batch_id: int, start_timestamp: float):
        assert (
            self._curr_swap_interval is not None
            and self._curr_swap_interval.start_swap_out_batch_id is not None
            and self._curr_swap_interval.start_swap_out_timestamp is not None
            and self._curr_swap_interval.start_swap_in_batch_id is None
            and self._curr_swap_interval.start_swap_in_timestamp is None
            and self._curr_swap_interval.finish_swap_in_batch_id is None
            and self._curr_swap_interval.finish_swap_in_batch_id is None
        )
    ...
    def finish_swap_in(self, batch_id: int, end_timestamp: float):
        assert (
            ...
            and self._curr_swap_interval.start_swap_out_batch_id is not None
            and self._curr_swap_interval.start_swap_out_timestamp is not None
            and self._curr_swap_interval.start_swap_in_batch_id is not None
            and self._curr_swap_interval.start_swap_in_batch_id is not None
            and self._curr_swap_interval.start_swap_in_timestamp is not None
            and self._curr_swap_interval.finish_swap_in_batch_id is None
            and self._curr_swap_interval.finish_swap_in_timestamp is None
        )
    ...
    def get_swap_durations(self) -> List[float]:
        ...
        return [
            interval.get_total_duration()
            for interval in self.swap_intervals
...
class WorkerMetricsStore:
    ...
    def on_swap_out_start(self, seq_id: str, num_tokens: int, start_timestamp: float):
        ...
        self.sequence_metrics[seq_id].start_swap_out(
            self.batch_metrics[-1].batch_id, num_tokens, start_timestamp
        )
    ...
    def on_swap_in_start(self, seq_id: str, start_timestamp: float):
        ...
        assert (
            ...
            and self.sequence_metrics[seq_id]._curr_swap_interval is not None
        )
        self.sequence_metrics[seq_id].start_swap_in(
            self.batch_metrics[-1].batch_id, start_timestamp
        )
    ...
    def on_swap_in_end(self, seq_id: str, end_timestamp: float):
        ...
        self.sequence_metrics[seq_id].finish_swap_in(
            self.batch_metrics[-1].batch_id, end_timestamp
        )
    ...
    def process_metrics(self) -> Dict[str, Any]:
        ...
        for seq_metrics in self.sequence_metrics.values():
            ...
            if seq_metrics.arrival_to_scheduled_delay is not None:
                all_arrival_to_scheduled_delays.append(
                    seq_metrics.arrival_to_scheduled_delay
                )
        ...
        benchmark_metrics["prefill_done_to_first_decode_delay"] = (
            calculate_percentile_values(
                [
                    seq_metrics.prefill_done_to_first_decode_delay
                    for seq_metrics in self.sequence_metrics.values()
                ]
            )
        )

        benchmark_metrics["arrival_to_first_decode_delay"] = (
            calculate_percentile_values(
                [
                    seq_metrics.arrival_to_first_decode_delay
                    for seq_metrics in self.sequence_metrics.values()
                ]
            )
        )
        ...
        benchmark_metrics["arrival_to_scheduled_delay"] = calculate_percentile_values(
            all_arrival_to_scheduled_delays
        )

        benchmark_metrics["engine_scheduler_latency"] = calculate_percentile_values(
            self.engine_scheduler_latencies
        )
        benchmark_metrics_raw["engine_scheduler_latency"] = (
            self.engine_scheduler_latencies
        )
        ...
        for seq_id, seq_metrics in self.sequence_metrics.items():
            ...
            seq_dict["num_decode_batches"] = sum(
                1
            ...
            if seq_metrics.batch_ids_scheduled:
                seq_dict["end_to_end_time"] = (
                    self.batch_metrics[
                        seq_metrics.batch_ids_scheduled[-1]
                    ].end_timestamp
            ...
            seq_dict["arrival_to_scheduled_delay"] = (
                seq_metrics.arrival_to_scheduled_delay
            )
            ...
            seq_dict["swap_duration_num_batches"] = calculate_percentile_values(
                swap_duration_num_batches
            )
            sequence_metrics_raw[seq_id]["swap_duration_num_batches"] = (
                swap_duration_num_batches
            )

            sequence_metrics_raw[seq_id]["arrival_to_first_decode_delay"] = (
                seq_metrics.arrival_to_first_decode_delay
            )

        benchmark_metrics["end_to_end_time"] = calculate_percentile_values(
            [
                seq_metrics["end_to_end_time"]
                for seq_metrics in sequence_metrics.values()
            ]
        )
